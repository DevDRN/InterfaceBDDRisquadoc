...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($errors)) {
        ...
        $upd = oci_parse($conn, 'UPDATE USERS SET PRENOM = :prenom, NOM = :nom, EMAIL = :email, USERNAME = :username, MDP = :mdp WHERE MATRICULE = :matricule');
        oci_bind_by_name($upd, ':nom',$nom);
        oci_bind_by_name($upd, ':prenom',$prenom);
        oci_bind_by_name($upd, ':email',$email);
        oci_bind_by_name($upd, ':username',$username);
        oci_bind_by_name($upd, ':mdp',$newMdp);
        oci_bind_by_name($upd, ':matricule',$matricule);
        $ok = oci_execute($upd, OCI_COMMIT_ON_SUCCESS);

        if ($ok) {
            $_SESSION['USERNAME'] = $username;
            $messages[] = 'Profil mis à jour.';
            $subject = "GIL - Reinitialisation de votre mot de passe";
            $body = "Bonjour $prenom,\r\n\r\n Votre mot de passe à été réinitialiser.\r\nSi vous n'êtes pas à l'origine de cette action, merci de le signaler imédiatement au responsable d'application. \r\n http://10.49.22.125/";
            if ($newMdp && !sendReinitPassword($email, $nom, $prenom,$username, $subject, $body)) {
                $errors[] = "Échec de l’envoi d’email de confirmation.";
            } 
        } else {
            $e = oci_error($upd);
            $errors[] = 'Erreur base de données: '.htmlspecialchars($e['message']);
        }
...
<body class='p-4'>
    ...
    <form action="" method="post">
        ...
        <div class="mb-3">
            <label for="">Nouveau mot de passe <small>(laisser vide si pas modifié)</small></label>
            <input type="password" name="mdp" class="form-control">
        </div>
        <button name="update_profile" type="submit" class="btn btn-primary">Mettre à jour</button>
