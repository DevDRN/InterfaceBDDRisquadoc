...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    $sqlDel = "DELETE FROM LABOS WHERE CODE_LABO = :id";
    ...
    if (empty($erreurs)) {
        $sqlUpd = "UPDATE LABOS
                    SET NOM_LABO = :nomLabo,
                        ...
                        REGION = :region,
                        PAYS = :pays,
                        TEL_GENERIQUE = :telGenerique,
                        FAX_GENERIQUE = :faxGenerique,
                        MAIL_GENE_1 = :mailGenA,
                        MAIL_GENE_2 = :mailGenB,
        ...
        oci_bind_by_name($stmtUpd, ':codePostal', $codePostal);
        oci_bind_by_name($stmtUpd, ':ville', $ville);
        oci_bind_by_name($stmtUpd, ':region', $region);
        oci_bind_by_name($stmtUpd, ':pays', $pays);
        oci_bind_by_name($stmtUpd, ':telGenerique', $telGenerique);
        oci_bind_by_name($stmtUpd, ':faxGenerique', $faxGenerique);
        oci_bind_by_name($stmtUpd, ':mailGenA', $mailGenA);
        oci_bind_by_name($stmtUpd, ':mailGenB', $mailGenB);
        oci_bind_by_name($stmtUpd, ':mailGenC', $mailGenC);
        oci_bind_by_name($stmtUpd, ':dateMaj', $dateMaj);
        oci_bind_by_name($stmtUpd, ':commentaire', $commentaire);

        $okUpd = oci_execute($stmtUpd, OCI_COMMIT_ON_SUCCESS);

        if($okUpd) {
            $success ='Le laboratoire a été mis à jour avec succès.';
        } else {
            $e = oci_error($stmtUpd);
...
//Récup des données de pré-remplissage
$sqlSel = "SELECT * FROM LABOS WHERE CODE_LABO = :id";
$stmtSel= oci_parse($conn, $sqlSel);
oci_bind_by_name($stmtSel,':id', $id);
oci_execute($stmtSel);
$row = oci_fetch_assoc($stmtSel);
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || isset($_POST['delete'])) {
    ...
    $region = $row['REGION'];
    $pays = $row['PAYS'];
    $telGenerique = $row['TEL_GENERIQUE'];
    $faxGenerique = $row['FAX_GENERIQUE'];
    $mailGenA = $row['MAIL_GENE_1'];
    $mailGenB = $row['MAIL_GENE_2'];
    $mailGenC = $row['MAIL_GENE_3'];
...
<head>
  ...
  <title>InterfaceBDDRisquadoc Modification laboratoire</title>
  ...
  <script type="text/javascript">
    ...
    function confirmDeletion() {
        return confirm("Êtes-vous sûr de vouloir supprimer ce laboratoire ? Cette action est irréversible.");
    }
    </script>
</head>
...
<!--     <div class="mb-3">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="codeGef" name="codeGef" value="<?= htmlspecialchars($codeGef ?? '') ?>">
    </div>
    <div class="mb-3">
      <label for="adresse" class="form-label">Adresse *</label>
      <input type="text" class="form-control" id="adresse" name="adresse" required value="<?= htmlspecialchars($adresse ?? '') ?>">
    </div>
    <div class="mb-3">
